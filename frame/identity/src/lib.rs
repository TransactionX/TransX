...
//! ## Overview
//!
//! A federated naming system,
// 一个命名系统
//allowing for multiple registrars to be added from a specified origin.
// 允许从一个指定的原点添加多个注册商
...
//! each of their fees).
 // 任何人都可以提出一个拟议的身份，作为一笔定期存款，并要求任何数量的登记机构进行审查(支付每个登记机构的费用)。
 // Registrar judgements are given as an `enum`, allowing for sophisticated,
//! multi-tier opinions.
//!
//! Some judgements are identified as *sticky*, which means they cannot be removed except by
//! complete removal of the identity, or by the registrar.
// 有些判决被确定为“粘滞性”，这意味着除非完全删除其身份或由书记官长删除，否则不能删除这些判决
// Judgements are allowed to represent a
...
// 登记机构的数量应受到限制，存款数额应足够大，以确保没有任何国家膨胀攻击是可行的。
//!
//! ## Interface
//!
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn request_judgement(origin,
			...
			match id.judgements.binary_search_by_key(&reg_index, |x| x.0) {
				// 如果已经被付费或是Judgement::Erroneous  就终止
		...
		fn provide_judgement(origin,
			#[compact] reg_index: RegistrarIndex, // 哪个
