...
// 		}
		...
		// 销毁国库的币
		#[weight = 50_000]
		fn burn_for_treasury(origin, balances: BalanceOf<T>) {
			/// 销毁国库的币
			ensure_root(origin)?;
			T::ProposalRejection::on_unbalanced(T::Currency::withdraw(&Self::account_id(), balances, WithdrawReason::Transfer.into(), KeepAlive)?);
			Self::deposit_event(RawEvent::Burn);
