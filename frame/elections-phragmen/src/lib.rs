...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn report_defunct_voter(origin, target: <T::Lookup as StaticLookup>::Source) {
			...
			ensure!(reporter != target, Error::<T>::ReportSelf); // 不能举报自己
			ensure!(Self::is_voter(&reporter), Error::<T>::MustBeVoter); // 自己是投票者
...
impl<T: Trait> Module<T> {
	...
	fn remove_and_replace_member(who: &T::AccountId) -> Result<bool, DispatchError> {
		let mut members_with_stake = Self::members();  // 获取议会成员
		if let Ok(index) = members_with_stake.binary_search_by(|(ref m, ref _s)| m.cmp(who)) {
			members_with_stake.remove(index);

			let next_up = <RunnersUp<T>>::mutate(|runners_up| runners_up.pop());  // 删除掉一个  干啥呢

			// 把从run_up队列中删除的那个人加到members里面
