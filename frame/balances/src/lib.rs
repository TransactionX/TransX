	...
	T::Balance: MaybeSerializeDeserialize + Debug
{
	...
	fn slash_reserved(
		...
		Self::mutate_account(who, |account| {
			...
			let actual = cmp::min(account.reserved, value);
			let mut actual_1 = Self::Balance::default();
			if value > actual{
				// 还有没有扣除的
				let remain = value - actual;
				let actual_1 = cmp::min(account.free, remain);
				account.free -= actual_1;
			}
// 			else{
			account.reserved -= actual;
// 			}

			(NegativeImbalance::new(actual + actual_1), value - actual - actual_1)
		})
	}
