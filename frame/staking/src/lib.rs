...
//! todo 赏罚分明  不旅行职责就会扣除资金
//! ### Terminology
//! <!-- Original author of paragraph: @gavofyork -->
//!
//! - Staking: The process of locking up funds for some time, placing them at risk of slashing
//! (loss) in order to become a rewarded maintainer of the network.
//!  todo 将自身资金锁定一段时间。 有回报也有风险
...
//! todo 这个era是多个session搞一次
//! validator's active nominator set) is recalculated and where rewards are paid out.
//!  todo 重新计算验证者集合和奖励的时间
...
//!
//! todo 成为验证者之前应该先成为候选人  然后由提名者的投票来决定
//!
//! An account can become a validator candidate via the
//! [`validate`](./enum.Call.html#variant.validate) call.
//!
//! #### Nomination  todo 提名者 这个只是用来投票 跟验证与候选不同
//!
//! A **nominator** does not take any _direct_ role in maintaining the network, instead, it votes on
//! a set of validators  to be elected. Once interest in nomination is stated by an account, it
//! takes effect at the next election round. The funds in the nominator's stash account indicate the
//! _weight_ of its vote.
//!  todo 提名者的资金就是它投票的 比重 就相当于话语权
//!
//! Both the rewards and any punishment that a validator earns are shared
//! between the validator and its nominators. This rule incentivizes the nominators to NOT vote for
//! the misbehaving/offline validators as much as possible, simply because the nominators will also
//! lose funds if they vote poorly.
//!  todo 问题： 什么时候提名者可以进行投票？？？？？？
...
pub type ValidatorIndex = u16;  // todo 说明验证节点最多2的16次方 65536个节点
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn offchain_worker(now: T::BlockNumber) {
			...
			if Self::era_election_status().is_open_at(now) {
				...
				if let Err(why) = offchain_status {
					...
					// todo 不用签名执行这个函数 submit_election_solution_unsigned
		...
		// todo 使结合起来  这里是绑定
			...
			system::Module::<T>::inc_ref(&stash);  // todo 这个操作有啥用呢
		...
		/// # </weight>
		#[weight = 750_000_000]
		// todo 为一个stash账号设置一个controller账号
...
impl<T: Trait> Module<T> {
	...
	/// Clears both snapshots of stakers.  // todo snapshots 快照
	fn kill_stakers_snapshot() {
		// todo 说明staker可能包括验证者和提名者
